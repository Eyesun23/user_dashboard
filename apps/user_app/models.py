...
class messageManager(models.Manager):
	...
	def retrieve_message(self, user_id):
		...
		data_2 = User.objects.raw("SELECT user_app_user.id, user_app_user.first_name, user_app_user.last_name, user_app_message.created_at, user_app_message.message, user_app_message.id AS 'message_id' FROM user_app_user JOIN user_app_message ON user_app_user.id = user_app_message.user_id WHERE user_app_message.to_who = {} ORDER BY user_app_message.created_at DESC".format(user_id))
...
class postManager(models.Manager):
	...
	def retrieve_post(self, user_id):
		messages = Message.objects.raw("SELECT user_app_message.id FROM user_app_message WHERE to_who = {}".format(user_id))
		message_ids = ""
		for message in messages:
			message_ids += str(message.id) + ", "
		message_ids = message_ids[:-2]
		posts = Post.objects.raw("SELECT user_app_post.id, user_app_user.first_name, user_app_user.last_name, user_app_post.post, user_app_post.created_at, user_app_post.user_id, user_app_post.message_id FROM user_app_post JOIN user_app_user ON user_app_post.user_id = user_app_user.id WHERE user_app_post.message_id IN ({})".format(message_ids))
		return posts
